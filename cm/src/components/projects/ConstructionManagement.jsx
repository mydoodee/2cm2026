//src/componentes/projects/ConstructionManagement.jsx
...
const ConstructionManagement = ({ user, setUser, theme, setTheme }) => {
  ...
  const fetchFolders = async () => {
    ...
    try {
      ...
      const designFolder = foldersWithPermission.find(f => f.folder_name === 'CM' && !f.parent_folder_id);
      if (designFolder) {
...
const handleNavigateImage = async (direction) => {
  ...
  const isRootDesignFolder = !folder.parent_folder_id && folder.folder_name === 'CM';
  
  if (isRootDesignFolder) {
    message.warning('ไม่สามารถสร้างโฟลเดอร์ในโฟลเดอร์หลัก CM ได้โดยตรง กรุณาสร้างใน subfolder');
    return;
  }
...
const handleOpenRenameFolderModal = (folderId, folderName) => {
  ...
  // ✅ ตรวจสอบว่าเป็น root CM หรือไม่
  const isRootDesignFolder = !folder.parent_folder_id && folder.folder_name === 'CM';
  
  if (isRootDesignFolder) {
    message.warning('ไม่สามารถเปลี่ยนชื่อโฟลเดอร์หลัก CM ได้');
    return;
  }
  
  // ✅ ตรวจสอบว่าเป็นโฟลเดอร์ลูกโดยตรงของ CM หรือไม่ (เช่น Drawings, Documents)
  if (folder.parent_folder_id) {
    const parentFolder = folders.find(f => String(f.folder_id) === String(folder.parent_folder_id));
    const isDirectChildOfRoot = parentFolder && !parentFolder.parent_folder_id && parentFolder.folder_name === 'CM';
    
    if (isDirectChildOfRoot) {
      message.warning('ไม่สามารถเปลี่ยนชื่อโฟลเดอร์หลักย่อยของ CM ได้ (เช่น Drawings, Documents)');
      return;
    }
  }
...
const canDelete = !isRootFolder && !hasSubfolders && ['write', 'admin'].includes(data.permission_type);
  ...
  const handleOpenUploadModal = (folderId, folderName) => {
    ...
    const isRootFolder = !folder.parent_folder_id && folder.folder_name === 'CM';
...
const buildTree = (allFolders, parentId) => {
  return allFolders
    ...
    .map(folder => {
      ...
      // ✅ ตรวจสอบว่าเป็นลูกโดยตรงของ root CM หรือไม่
...
const designRoots = uniqueFolders.filter(f => f.folder_name === 'CM' && !f.parent_folder_id && f.permission_type && ['read', 'write', 'admin'].includes(f.permission_type));
                                       ...
                                       <div className="relative z-10 p-4 md:p-6">
                                         <Space direction="vertical" size="small" style={{ width: '100%' }}>
                                           ...
                                           <Text className="text-gray-200">
                                             จัดการเอกสารควบคุมการก่อสร้างและรายงานโครงการ
                                           </Text>
                                           <Button
                                             type="primary"
                                             icon={<LeftOutlined />}
...
>
  <Form layout="vertical" className="font-kanit">
    ...
    {/* <Text className={`${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'} text-xs`}>
      💡 ไม่สามารถเปลี่ยนชื่อโฟลเดอร์หลัก CM และโฟลเดอร์ย่อยระดับแรก (เช่น Drawings, Documents) ได้
    </Text> */}
  </Form>
</Modal>
    </div>
