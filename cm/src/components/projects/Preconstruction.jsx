//src/componentes/projects/Preconstruction.jsx
...
const Preconstruction = ({ user, setUser, theme, setTheme }) => {
  ...
  const fetchFolders = async () => {
    ...
    try {
      ...
      const designFolder = foldersWithPermission.find(f => f.folder_name === 'PreConstruction' && !f.parent_folder_id);
      if (designFolder) {
...
const handleNavigateImage = async (direction) => {
  ...
  const isRootDesignFolder = !folder.parent_folder_id && folder.folder_name === 'PreConstruction';
  
  if (isRootDesignFolder) {
    message.warning('ไม่สามารถสร้างโฟลเดอร์ในโฟลเดอร์หลัก PreConstruction ได้โดยตรง กรุณาสร้างใน subfolder');
    return;
  }
...
const handleOpenRenameFolderModal = (folderId, folderName) => {
  ...
  // ✅ ตรวจสอบว่าเป็น root PreConstruction หรือไม่
  const isRootDesignFolder = !folder.parent_folder_id && folder.folder_name === 'PreConstruction';
  
  if (isRootDesignFolder) {
    message.warning('ไม่สามารถเปลี่ยนชื่อโฟลเดอร์หลัก PreConstruction ได้');
    return;
  }
  
  // ✅ ตรวจสอบว่าเป็นโฟลเดอร์ลูกโดยตรงของ PreConstruction หรือไม่ (เช่น Drawings, Documents)
  if (folder.parent_folder_id) {
    const parentFolder = folders.find(f => String(f.folder_id) === String(folder.parent_folder_id));
    const isDirectChildOfRoot = parentFolder && !parentFolder.parent_folder_id && parentFolder.folder_name === 'PreConstruction';
    
    if (isDirectChildOfRoot) {
      message.warning('ไม่สามารถเปลี่ยนชื่อโฟลเดอร์หลักย่อยของ PreConstruction ได้ (เช่น Drawings, Documents)');
      return;
    }
  }
...
const canDelete = !isRootFolder && !hasSubfolders && ['write', 'admin'].includes(data.permission_type);
  ...
  const handleOpenUploadModal = (folderId, folderName) => {
    ...
    const isRootFolder = !folder.parent_folder_id && folder.folder_name === 'PreConstruction';
...
const buildTree = (allFolders, parentId) => {
  return allFolders
    ...
    .map(folder => {
      ...
      // ✅ ตรวจสอบว่าเป็นลูกโดยตรงของ root PreConstruction หรือไม่
...
const designRoots = uniqueFolders.filter(f => f.folder_name === 'PreConstruction' && !f.parent_folder_id && f.permission_type && ['read', 'write', 'admin'].includes(f.permission_type));
            ...
            <div className="relative z-10 p-4 md:p-6">
              ...
              <Title level={2} className={`text-shadow-lg ${theme === 'dark' ? 'text-white' : 'text-white'}`}>
                ...
                <Text className={`${theme === 'dark' ? 'text-gray-300' : 'text-gray-200'}`}>
                  จัดการไฟล์ preconstruction และเอกสารโครงการ
                </Text>
                <Button
                  type="primary"
                  icon={<LeftOutlined />}
...
>
  <Form layout="vertical" className="font-kanit">
    ...
    {/* <Text className={`${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'} text-xs`}>
      💡 ไม่สามารถเปลี่ยนชื่อโฟลเดอร์หลัก PreConstruction และโฟลเดอร์ย่อยระดับแรก (เช่น Drawings, Documents) ได้
    </Text> */}
  </Form>
</Modal>
    </div>
