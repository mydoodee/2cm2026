// src/components/projects/Preconstruction.jsx
...
const Preconstruction = ({ user, setUser, theme, setTheme }) => {
  ...
  const handleOpenCreateFolderModal = (parentFolderId, parentFolderName) => {
    ...
    if (!folder.parent_folder_id && folder.folder_name === 'PreConstruction') { message.warning('ไม่สามารถสร้างโฟลเดอร์ในโฟลเดอร์หลัก PreConstruction ได้โดยตรง'); return; }
  ...
  const handleOpenRenameFolderModal = (folderId, folderName) => {
    ...
    if (!folder.parent_folder_id && folder.folder_name === 'PreConstruction') { message.warning('ไม่สามารถเปลี่ยนชื่อโฟลเดอร์หลัก PreConstruction ได้'); return; }
    if (folder.parent_folder_id) {
      const parentFolder = folders.find(f => String(f.folder_id) === String(folder.parent_folder_id));
      if (parentFolder && !parentFolder.parent_folder_id && parentFolder.folder_name === 'PreConstruction') {
        message.warning('ไม่สามารถเปลี่ยนชื่อโฟลเดอร์ย่อยระดับแรกของ PreConstruction ได้'); return;
      }
    }
  ...
  const handleOpenUploadModal = (folderId, folderName) => {
    ...
    if (!folder.parent_folder_id && folder.folder_name === 'PreConstruction') { message.warning('ไม่สามารถอัพโหลดไฟล์ในโฟลเดอร์หลักได้ กรุณาเลือก subfolder'); return; }
    ...
    <div className={`${theme === 'dark' ? 'bg-gray-900' : 'bg-gray-50'} font-kanit`} style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
              ...
              <Space direction="vertical" size="small" style={{ width: '100%' }}>
                <Breadcrumb 
                  items={[
                    ...
                    { title: <span className="!text-white font-bold">PreConstruction</span> }
                  ]} 
      ...
      <div style={{ flex: 1, padding: '16px', display: 'flex', flexDirection: 'column', minHeight: 0 }}>
        <DriveUI
          ...
          rootFolderName="PreConstruction"
