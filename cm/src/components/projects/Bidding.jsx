...
const Bidding = ({ user, setUser, theme, setTheme }) => {
  ...
  const fetchFolders = async () => {
    ...
    try {
      ...
      const designFolder = foldersWithPermission.find(f => f.folder_name === 'Bidding' && !f.parent_folder_id);
  ...
  const handleOpenCreateFolderModal = (parentFolderId, parentFolderName) => {
    ...
    if (!folder.parent_folder_id && folder.folder_name === 'Bidding') { message.warning('ไม่สามารถสร้างโฟลเดอร์ในโฟลเดอร์หลัก Bidding ได้โดยตรง'); return; }
  ...
  const handleOpenRenameFolderModal = (folderId, folderName) => {
    ...
    if (!folder.parent_folder_id && folder.folder_name === 'Bidding') { message.warning('ไม่สามารถเปลี่ยนชื่อโฟลเดอร์หลัก Bidding ได้'); return; }
    if (folder.parent_folder_id) {
      const parentFolder = folders.find(f => String(f.folder_id) === String(folder.parent_folder_id));
      if (parentFolder && !parentFolder.parent_folder_id && parentFolder.folder_name === 'Bidding') {
        message.warning('ไม่สามารถเปลี่ยนชื่อโฟลเดอร์ย่อยระดับแรกของ Bidding ได้'); return;
      }
    }
  ...
  const handleOpenUploadModal = (folderId, folderName) => {
    ...
    if (!folder.parent_folder_id && folder.folder_name === 'Bidding') { message.warning('ไม่สามารถอัพโหลดไฟล์ในโฟลเดอร์หลักได้ กรุณาเลือก subfolder'); return; }
