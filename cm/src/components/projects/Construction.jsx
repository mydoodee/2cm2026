// src/components/projects/Construction.jsx
...
const Construction = ({ user, setUser, theme, setTheme }) => {
  ...
  const handleOpenCreateFolderModal = (parentFolderId, parentFolderName) => {
    ...
    if (!folder.parent_folder_id && folder.folder_name === 'Construction') { message.warning('ไม่สามารถสร้างโฟลเดอร์ในโฟลเดอร์หลัก Construction ได้โดยตรง'); return; }
  ...
  const handleOpenRenameFolderModal = (folderId, folderName) => {
    ...
    if (!folder.parent_folder_id && folder.folder_name === 'Construction') { message.warning('ไม่สามารถเปลี่ยนชื่อโฟลเดอร์หลัก Construction ได้'); return; }
    if (folder.parent_folder_id) {
      const parentFolder = folders.find(f => String(f.folder_id) === String(folder.parent_folder_id));
      if (parentFolder && !parentFolder.parent_folder_id && parentFolder.folder_name === 'Construction') {
        message.warning('ไม่สามารถเปลี่ยนชื่อโฟลเดอร์ย่อยระดับแรกของ Construction ได้'); return;
      }
    }
  ...
  const handleOpenUploadModal = (folderId, folderName) => {
    ...
    if (!folder.parent_folder_id && folder.folder_name === 'Construction') { message.warning('ไม่สามารถอัพโหลดไฟล์ในโฟลเดอร์หลักได้ กรุณาเลือก subfolder'); return; }
              ...
              <Space direction="vertical" size="small" style={{ width: '100%' }}>
                <Breadcrumb 
                  items={[
                    ...
                    { title: <span className="!text-white font-bold">Construction</span> }
                  ]} 
                ...
                <Text className="!text-gray-200">
                  จัดการไฟล์ construction และเอกสารโครงการ
                </Text>
